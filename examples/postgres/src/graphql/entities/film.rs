use crate::graphql::enums::MpaaRating;
use sea_orm::prelude::*;
pub fn filter_recursive(root_filter: Option<Filter>) -> sea_orm::Condition {
    ...
    if let Some(current_filter) = root_filter {
        ...
        if let Some(fulltext) = current_filter.fulltext {
            if let Some(eq_value) = fulltext.eq {
                condition = condition.add(Column::Fulltext.eq(eq_value))
            }
            if let Some(ne_value) = fulltext.ne {
                condition = condition.add(Column::Fulltext.ne(ne_value))
            }
            if let Some(gt_value) = fulltext.gt {
                condition = condition.add(Column::Fulltext.gt(gt_value))
            }
            if let Some(gte_value) = fulltext.gte {
                condition = condition.add(Column::Fulltext.gte(gte_value))
            }
            if let Some(lt_value) = fulltext.lt {
                condition = condition.add(Column::Fulltext.lt(lt_value))
            }
            if let Some(lte_value) = fulltext.lte {
                condition = condition.add(Column::Fulltext.lte(lte_value))
            }
            if let Some(is_in_value) = fulltext.is_in {
                condition = condition.add(Column::Fulltext.is_in(is_in_value))
            }
            if let Some(is_not_in_value) = fulltext.is_not_in {
                condition = condition.add(Column::Fulltext.is_not_in(is_not_in_value))
            }
            if let Some(is_null_value) = fulltext.is_null {
                if is_null_value {
                    condition = condition.add(Column::Fulltext.is_null())
                }
            }
        }
    }
...
impl Model {
    ...
    pub async fn release_year(&self) -> &Option<i32> {
        &self.release_year
    }
    pub async fn language_id(&self) -> &i16 {
        &self.language_id
    }
    pub async fn original_language_id(&self) -> &Option<i16> {
        &self.original_language_id
    }
    pub async fn rental_duration(&self) -> &i16 {
        &self.rental_duration
    }
    pub async fn rental_rate(&self) -> &Decimal {
        &self.rental_rate
    }
    pub async fn length(&self) -> &Option<i16> {
        &self.length
    }
    pub async fn replacement_cost(&self) -> &Decimal {
        &self.replacement_cost
    }
    pub async fn rating(&self) -> Option<MpaaRating> {
    ...
    pub async fn fulltext(&self) -> &String {
        &self.fulltext
    }
...
pub struct Filter {
    ...
    pub film_id: Option<TypeFilter<i32>>,
    pub title: Option<TypeFilter<String>>,
    pub description: Option<TypeFilter<String>>,
    pub release_year: Option<TypeFilter<i32>>,
    pub language_id: Option<TypeFilter<i16>>,
    pub original_language_id: Option<TypeFilter<i16>>,
    pub rental_duration: Option<TypeFilter<i16>>,
    pub rental_rate: Option<TypeFilter<Decimal>>,
    pub length: Option<TypeFilter<i16>>,
    ...
    pub fulltext: Option<TypeFilter<String>>,
}
#[derive(Clone, Eq, PartialEq, Hash, Debug)]
pub struct FilmInventoryFK(i32);
...
#[derive(Clone, Eq, PartialEq, Hash, Debug)]
pub struct LanguageLanguageFK(i16);
#[async_trait::async_trait]
...
#[derive(Clone, Eq, PartialEq, Hash, Debug)]
pub struct OriginalLanguageLanguageFK(Option<i16>);
#[async_trait::async_trait]
impl async_graphql::dataloader::Loader<OriginalLanguageLanguageFK> for OrmDataloader {
    ...
    ) -> Result<std::collections::HashMap<OriginalLanguageLanguageFK, Self::Value>, Self::Error>
    {
        ...
        Ok(crate::orm::language::Entity::find()
            ...
            .map(|model| {
                let key =
                    OriginalLanguageLanguageFK(Some(model.language_id.clone().try_into().unwrap()));
                (key, model)
            })
            .collect())
    }
...
#[derive(Clone, Eq, PartialEq, Hash, Debug)]
pub struct FilmFilmActorFK(i32);
#[async_trait::async_trait]
...
#[derive(Clone, Eq, PartialEq, Hash, Debug)]
pub struct FilmFilmCategoryFK(i32);
#[async_trait::async_trait]
