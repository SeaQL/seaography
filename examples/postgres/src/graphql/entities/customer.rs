...
pub fn filter_recursive(root_filter: Option<Filter>) -> sea_orm::Condition {
    ...
    if let Some(current_filter) = root_filter {
        ...
        if let Some(activebool) = current_filter.activebool {
            if let Some(eq_value) = activebool.eq {
                condition = condition.add(Column::Activebool.eq(eq_value))
            }
            if let Some(ne_value) = activebool.ne {
                condition = condition.add(Column::Activebool.ne(ne_value))
            }
            if let Some(gt_value) = activebool.gt {
                condition = condition.add(Column::Activebool.gt(gt_value))
            }
            if let Some(gte_value) = activebool.gte {
                condition = condition.add(Column::Activebool.gte(gte_value))
            }
            if let Some(lt_value) = activebool.lt {
                condition = condition.add(Column::Activebool.lt(lt_value))
            }
            if let Some(lte_value) = activebool.lte {
                condition = condition.add(Column::Activebool.lte(lte_value))
            }
            if let Some(is_in_value) = activebool.is_in {
                condition = condition.add(Column::Activebool.is_in(is_in_value))
            }
            if let Some(is_not_in_value) = activebool.is_not_in {
                condition = condition.add(Column::Activebool.is_not_in(is_not_in_value))
            }
            if let Some(is_null_value) = activebool.is_null {
                if is_null_value {
                    condition = condition.add(Column::Activebool.is_null())
                }
            }
        }
...
impl Model {
    ...
    pub async fn activebool(&self) -> &bool {
        &self.activebool
    }
    pub async fn create_date(&self) -> &Date {
        &self.create_date
    }
    pub async fn last_update(&self) -> &Option<DateTime> {
        &self.last_update
    }
    pub async fn active(&self) -> &Option<i32> {
        &self.active
    }
...
pub struct Filter {
    ...
    pub activebool: Option<TypeFilter<bool>>,
    pub create_date: Option<TypeFilter<Date>>,
    pub last_update: Option<TypeFilter<DateTime>>,
    pub active: Option<TypeFilter<i32>>,
}
#[derive(Clone, Eq, PartialEq, Hash, Debug)]
pub struct CustomerPaymentFK(i32);
...
#[derive(Clone, Eq, PartialEq, Hash, Debug)]
pub struct CustomerRentalFK(i32);
#[async_trait::async_trait]
