...
impl Model {
    pub async fn payment_id(&self) -> &i32 {
        &self.payment_id
    }
    pub async fn customer_id(&self) -> &i16 {
        &self.customer_id
    }
    pub async fn staff_id(&self) -> &i16 {
        &self.staff_id
    }
...
pub struct Filter {
    ...
    pub payment_id: Option<TypeFilter<i32>>,
    pub customer_id: Option<TypeFilter<i16>>,
    pub staff_id: Option<TypeFilter<i16>>,
...
pub struct CustomerCustomerFK(i16);
...
impl async_graphql::dataloader::Loader<RentalRentalFK> for OrmDataloader {
    ...
    async fn load(
        ...
        Ok(crate::orm::rental::Entity::find()
            ...
            .into_iter()
            .map(|model| {
                let key = RentalRentalFK(model.rental_id.clone().try_into().unwrap());
                (key, model)
            })
            .collect())
    }
