...
pub fn filter_recursive(root_filter: Option<Filter>) -> sea_orm::Condition {
    ...
    if let Some(current_filter) = root_filter {
        ...
        if let Some(location) = current_filter.location {
            if let Some(eq_value) = location.eq {
                condition = condition.add(Column::Location.eq(eq_value))
            }
            if let Some(ne_value) = location.ne {
                condition = condition.add(Column::Location.ne(ne_value))
            }
            if let Some(gt_value) = location.gt {
                condition = condition.add(Column::Location.gt(gt_value))
            }
            if let Some(gte_value) = location.gte {
                condition = condition.add(Column::Location.gte(gte_value))
            }
            if let Some(lt_value) = location.lt {
                condition = condition.add(Column::Location.lt(lt_value))
            }
            if let Some(lte_value) = location.lte {
                condition = condition.add(Column::Location.lte(lte_value))
            }
            if let Some(is_in_value) = location.is_in {
                condition = condition.add(Column::Location.is_in(is_in_value))
            }
            if let Some(is_not_in_value) = location.is_not_in {
                condition = condition.add(Column::Location.is_not_in(is_not_in_value))
            }
            if let Some(is_null_value) = location.is_null {
                if is_null_value {
                    condition = condition.add(Column::Location.is_null())
                }
            }
        }
...
pub struct Filter {
    ...
    pub location: Option<TypeFilter<String>>,
